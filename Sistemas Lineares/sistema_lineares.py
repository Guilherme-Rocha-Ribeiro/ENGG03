...
# x = A(inversa) * B
def solucao_geral(matriz_coeficientes: list[list[float]], matriz_independentes: list[list[float]]) -> list[list[float]]:
    A_inversa = np.linalg.inv(matriz_coeficientes)
    solucao = np.dot(A_inversa, matriz_independentes) # np.dot() é o metodo de multiplicacao de matrizes 
    return solucao
...
def linalg_solver(matriz_coeficientes: list[list[float]], matriz_independentes: list[list[float]]) -> list[list[float]]:
    return np.linalg.solve(matriz_coeficientes, matriz_independentes)
